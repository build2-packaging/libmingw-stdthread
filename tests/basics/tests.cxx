...
mingw_stdthread::shared_mutex sm;
mingw_stdthread::condition_variable cv;
mingw_stdthread::condition_variable_any cv_any;
...
int main()
{
  ...
  namespace this_thread = mingw_stdthread::this_thread;
    ...
    TEST_SL_MV_CPY(mingw_stdthread::mutex)
    TEST_SL_MV_CPY(mingw_stdthread::recursive_mutex)
    TEST_SL_MV_CPY(mingw_stdthread::timed_mutex)
    TEST_SL_MV_CPY(mingw_stdthread::recursive_timed_mutex)
    TEST_SL_MV_CPY(mingw_stdthread::shared_mutex)
    TEST_SL_MV_CPY(mingw_stdthread::shared_timed_mutex)
    TEST_SL_MV_CPY(mingw_stdthread::condition_variable)
    TEST_SL_MV_CPY(mingw_stdthread::condition_variable_any)
...
//  Regression test: Thread must copy any argument that is passed by value.
    ...
    thread t([](TestMove&& a, const char* b, int c) mutable
    {
        try
        {
...
//  Thread might move the string more than once.
            ...
            {
                std::lock_guard<mingw_stdthread::mutex> lock(m);
                cond = 1;
            ...
            {
                std::lock_guard<decltype(sm)> lock(sm);
                cond = 2;
                log("Notifying condvar");
                cv_any.notify_all();
            }
